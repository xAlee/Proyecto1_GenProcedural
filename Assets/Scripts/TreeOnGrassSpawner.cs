...
public class TreeOnGrassSpawner : MonoBehaviour
{
    ...
    [Header("ÁNCLAS PARA ANTENAS")]
    [Tooltip("Tag que identifica las casas donde se montarán las antenas")]
    ...
    void Start()
    {
        // Si el grid ya generó, dispara; si no, igual podrás usar Houses en contexto Antennas
    ...
    public void ApplyCustomRuleToForest(string fRule)
    {
        ...
        {
            Debug.LogWarning("[Spawner] Regla vacía. No se aplicó.");
            return;
        }
    ...
    private void DoSpawn()
    {
        ...
        // elegir prefab y bases según contexto
        ...
        if (bases.Count == 0)
        {
            Debug.LogWarning($"[Spawner] No se encontraron bases para {context}. " +
                             (context == SpawnContext.Antennas ? $"¿Tienen tag '{houseTag}'?" : "¿Grid generó bloques?"));
            return;
        }
        ...
        foreach (var b in bases)
        {
            ...
            // parent: para árboles, al bloque de grid; para antenas, a la casa
